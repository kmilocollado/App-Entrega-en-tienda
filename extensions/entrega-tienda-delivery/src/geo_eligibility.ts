...
/** CP (y opcionalmente ciudad) del punto de recogida; para no confundir la dirección del grupo con la del cliente. */
...
/**
 * Cuando Shopify mezcla recogida y envío, `deliveryAddress` del grupo suele seguir siendo la tienda.
 * Si viene el CP del JSON (`storeAddress.zip`) y choca con facturación distinta, usamos facturación.
 */
...
export function inCountry(addr: Addr, cfg: GeoMatchConfig): boolean {
  ...
  // Shopify a veces omite countryCode en deliveryAddress durante el checkout.
  if (!have && want === "es") {
